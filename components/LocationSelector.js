...
const style= {
    wrapper: `pt-2`,
    searchHeader: `w-full font-bold text-right items-center text-3xl p-4 overflow-hidden`,
    ...
    svgContainer: `mx-1`,
    input: `my-2 rounded-2 p-2 outline-none border-none bg-transparent  h-full w-full text-right`,
    verticalLine: `w-0 h-[2rem] border-black border absolute z-10 left-[2.3rem] top-[2rem]`,
}
const LocationSelector = () => {
  const [inFocus, setInFocus] = useState('from')
  const { pickup, setPickup, dropoff, setDropoff } = useContext(TaxiContext)
  return (
    <div className={style.wrapper}>
        <div className={style.searchHeader}>
            {inFocus === 'from' ? 'مبدا را وارد کنید' : 'مقصد را وارد کنید'}
        </div>
